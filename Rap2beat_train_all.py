...
class AudioInversionDataset(Dataset):
    ...
    def __getitem__(self, i):    
        ...
        if "melody" in self.config['condition_type']:
            melody_path = build_path("", audio_path)
        ...
        if "rhythm" in self.config['condition_type']:
            rhythm_path = build_path("/mnt/gestalt/home/yianlai0327/data/rhythm_feature", audio_path)
            rhythm_curve = load_npy(rhythm_path)
            # transpose to (T, 2) if needed
            if rhythm_curve.shape[1] != 2:
                rhythm_curve = rhythm_curve.T
                
            # pad to 4756
            if rhythm_curve.shape[0] < 4756:
                pad_width = 4756 - rhythm_curve.shape[0]
                rhythm_curve = np.pad(rhythm_curve, ((0, pad_width), (0, 0)), mode='constant')    
            else:
                rhythm_curve = rhythm_curve[:4756, :]
        else:
            rhythm_curve = np.zeros((4756, 2))
            
        # Load dynamics curve
        if "dynamics" in self.config['condition_type']:
            dynamics_path = build_path("/mnt/gestalt/home/huan/ntu/DeepMIR/RapBank/vocal_rms_npy", audio_path)
            dynamics_curve = load_npy(dynamics_path)
            # pad to 13108
            if dynamics_curve.shape[0] < 13108:
                pad_width = 13108 - dynamics_curve.shape[0]
                dynamics_curve = np.pad(dynamics_curve, (0, pad_width), mode='constant')
            else:
                dynamics_curve = dynamics_curve[:13108]
        else:
        ...
        # audio_token_path = os.path.join(self.audio_codec_root, audio_path.replace('wav', 'pth'))
        # audio = torch.load(audio_token_path, map_location=torch.device('cpu'))
        
        drum_token_path = os.path.join(self.audio_codec_root, audio_path.replace('wav', 'pt'))
        # print("drum_token_path", drum_token_path)
        # print("vocal_codec_root", self.vocal_codec_root)
        drum_latent = torch.load(drum_token_path, map_location='cpu')
        if drum_latent.dim() == 3: drum_latent = drum_latent.squeeze(0)
        
        vocal_token_path = os.path.join(self.vocal_codec_root, audio_path.replace('wav', 'pt'))
...
class CollateFunction:
    ...
    def __call__(self, examples):
        ...
        vocal_latents = [example["vocal_latent"] for example in examples]
        ...
        if len(self.condition_type) != 0:
            ...
            vocal_latents = torch.stack(vocal_latents).float()
            batch = {
                "audio_full_path": audio_full_path,
                "audio": audio,                 # target
                "vocal_latent": vocal_latents,  # condition
...
def log_validation(val_dataloader, condition_extractors, condition_type, pipeline, config, weight_dtype, global_step):
    ...
    for step, batch in enumerate(val_dataloader):
        ...
        ### conditioned
        extracted_melody_condition = condition_extractors["melody"](melody_condition.to(torch.float32).to("cuda"))
        extracted_dynamics_condition = condition_extractors["dynamics"](dynamics_condition.to(torch.float32).to("cuda"))
        extracted_rhythm_condition = condition_extractors["rhythm"](rhythm_condition.to(torch.float32).to("cuda"))
        ...
        vocal_latents = batch["vocal_latent"].to("cuda") 
        desired_repeats = 192 // 64  
        ...
        null_condition = torch.zeros_like(extracted_condition)
        extracted_condition = torch.cat([null_condition, null_condition, extracted_condition], dim=0)
        ...
        # 6. 儲存與評估
        # 轉回 Numpy
        dynamics_condition_np = dynamics_condition[0].detach().cpu().numpy() # 這是 Vocal RMS
        rhythm_condition_np = rhythm_condition[0].detach().cpu().numpy()     # 這是 Vocal Onset + Grid
        
        output = audio[0].T.float().cpu().numpy() # Generated Drums
        
        gen_file = os.path.join(val_audio_dir, f"validation_{step}.wav")
        original_file = os.path.join(val_audio_dir, f"original_drum_{step}.wav") # GT Drums
        
        sf.write(gen_file, output, pipeline.vae.sampling_rate)
        shutil.copy(audio_full_path[0], original_file)

        # A. 評估 Dynamics (Gen Drums vs GT Drums)
        # 我們希望生成的鼓在大聲的地方跟原本的鼓一樣大聲
        if "dynamics" in condition_type:
            print("Evaluating dynamics for step", step)
            gen_dynamics = compute_dynamics(gen_file)
            original_dynamics = compute_dynamics(original_file)
            
            plt.figure(dpi=200)
            plt.plot(gen_dynamics.squeeze(), label='Generated Drums', linewidth=1, alpha=0.8)
            plt.plot(original_dynamics.squeeze(), label="GT Drums", linewidth=1, alpha=0.8)
            # 選項：也可以把 Vocal RMS 畫上去當參考 (需縮放)
            # plt.plot(dynamics_condition_np.squeeze(), label="Vocal RMS (Cond)", linewidth=1, linestyle="--", alpha=0.5)
            
            plt.xlabel('Time Frame')
            plt.ylabel('Dynamics (dB)')
            plt.legend(fontsize=8) 
            plt.title(f"Dynamics Check - Step {step}")
        ...
        # B. 評估 Rhythm (Gen Drums vs Input Grid)
        # 這是最重要的！檢查生成的鼓有沒有打在 Grid (Channel 1) 上
        if "rhythm" in condition_type:
            print("Evaluating rhythm for step", step)
            processor = RNNDownBeatProcessor()
            generated_probabilities = processor(gen_file) # 偵測生成鼓的拍子
            
            # 這裡我們拿 Input Condition (Grid) 當作 Ground Truth 來比對
            # rhythm_condition_np 是 (2, T)，轉置後變成 (T, 2)
            input_probabilities = rhythm_condition_np.transpose(1, 0)
            # original_duration = batch["seconds_end"][0].item()
            # valid_frames = int(original_duration * frame_rate)
            # valid_frames = min(valid_frames, input_probabilities.shape[0])
            valid_frames = generated_probabilities.shape[0]
            input_probabilities = input_probabilities[:valid_frames, :]
            
            # 計算 F1
            hmm_processor = DBNDownBeatTrackingProcessor(beats_per_bar=[2, 3, 4], fps=100)
            # 注意：如果 input_probabilities 裡面的 beat grid 太乾淨(只有0和1)，madmom 可能會報錯
            # 但如果你有做高斯模糊就還好。如果報錯，可以用 try-except 包起來
            try:
            ...
            # 畫圖：這是檢查 Alignment 最直觀的方式
            frame_rate = 100
            time_axis = np.linspace(0, len(input_probabilities) / frame_rate, len(input_probabilities))
            
            fig, axes = plt.subplots(2, 1, figsize=(10, 8))
            
            # 上圖：輸入的條件 (Vocal Onset & Beat Grid)
            # axes[0].plot(time_axis, input_probabilities[:, 0], label="Vocal Onset (Cond)", color='blue', alpha=0.6)
            axes[0].plot(time_axis, input_probabilities[:, 1], label="Beat Grid (Cond)", color='green', alpha=0.8)
            axes[0].set_title("Input Conditions")
            axes[0].legend()
            
            # 下圖：生成的鼓 (Beat Probability)
            # 如果模型學得好，下圖的波峰應該要跟上圖的 Grid (綠線) 對齊
            axes[1].plot(time_axis, generated_probabilities[:, 0], label="Gen Drum Beat Prob", color='orange')
            axes[1].plot(time_axis, generated_probabilities[:, 1], label="Gen Drum Downbeat Prob", color='red', alpha=0.8)
            axes[1].set_title("Generated Output (What model produced)")
            axes[1].legend()
    ...
    # 計算平均並回傳
    avg_dyn = np.mean(score_dynamics) if score_dynamics else 0
    avg_mel = np.mean(score_melody) if score_melody else 0
    avg_rhy = np.mean(score_rhythm) if score_rhythm else 0
    
    print(f"Validation Step {global_step}: Dyn={avg_dyn:.3f}, Rhy={avg_rhy:.3f}")
    
    torch.cuda.empty_cache()
    gc.collect()
    return avg_dyn, avg_mel, avg_rhy
...
def main():
    ...
    if config["transformer_ckpt"] is not None:
        # print(f"Loading transformer checkpoint from {config['transformer_ckpt']}...")
